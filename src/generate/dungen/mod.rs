...
impl Dir {
    ...
    pub fn get_random_dir<'a>() -> &'a Dir {
        Dir::iterator()
            .nth(rng::inclusive_random(0, 3) as usize)
            .unwrap()
    }
}
...
impl Dungeon {
    ...
    fn has_exits(&mut self) -> bool {
        for _i in 0..1000 {
            ...
            let x: isize = rng::inclusive_random(
                self.exits[r as usize].x,
                self.exits[r as usize].x + self.exits[r as usize].width - 1,
            );
            let y: isize = rng::inclusive_random(
                self.exits[r as usize].y,
                self.exits[r as usize].y + self.exits[r as usize].height - 1,
            );
    ...
    fn create_feature(&mut self, x: isize, y: isize, dir: &Dir) -> bool {
        ...
        if self.get_tile(x as isize + dx, y as isize + dy) != Tile::Floor
            && self.get_tile(x as isize + dx, y as isize + dy) != Tile::Corridor
        {
            return false;
        }
    ...
    fn make_room(&mut self, x: isize, y: isize, dir: &Dir, firstroom: bool) -> bool {
        ...
        let mut room: Rect = Rect::new(
            0,
            0,
            rng::inclusive_random(minsize, maxsize),
            rng::inclusive_random(minsize, maxsize),
        );
        ...
        if self.place_rect(&room, Tile::Floor) {
            ...
            if *dir != Dir::South || firstroom {
                self.exits
                    .push(Rect::new(room.x, room.y - 1, room.width, 1));
            }

            if *dir != Dir::North || firstroom {
                self.exits
                    .push(Rect::new(room.x, room.y + room.height, room.width, 1));
            }

            if *dir != Dir::East || firstroom {
                self.exits
                    .push(Rect::new(room.x - 1, room.y, 1, room.height));
            }

            if *dir != Dir::West || firstroom {
                self.exits
                    .push(Rect::new(room.x + room.width, room.y, 1, room.height));
            }
    ...
    fn make_corridor(&mut self, x: isize, y: isize, dir: &Dir) -> bool {
        ...
        if self.place_rect(&corridor, Tile::Corridor) {
            if *dir != Dir::South && corridor.width != 1 {
                // north side
                self.exits
                    .push(Rect::new(corridor.x, corridor.y - 1, corridor.width, 1));
            }

            if *dir != Dir::North && corridor.width != 1 {
                // south side
                self.exits.push(Rect::new(
                    corridor.x,
            ...
            if *dir != Dir::East && corridor.height != 1 {
                // west side
                self.exits
                    .push(Rect::new(corridor.x - 1, corridor.y, 1, corridor.height));
            }

            if *dir != Dir::West && corridor.height != 1 {
                // east side
                self.exits.push(Rect::new(
    ...
    fn place_rect(&mut self, rect: &Rect, tile: Tile) -> bool {
        ...
        if (rect.x <= 1)
            || (rect.y <= 1)
            || (rect.x + rect.width > self.width - 1)
    ...
    fn place_object(&mut self, tile: Tile) -> bool {
        ...
        let x: isize = rng::inclusive_random(
            self.rooms[r as usize].x + 1,
            self.rooms[r as usize].x + self.rooms[r as usize].width - 2,
        );
        let y: isize = rng::inclusive_random(
            self.rooms[r as usize].y + 1,
            self.rooms[r as usize].y + self.rooms[r as usize].height - 2,
        );
